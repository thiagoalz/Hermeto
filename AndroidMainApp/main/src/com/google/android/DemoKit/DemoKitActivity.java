...
public class DemoKitActivity extends Activity implements Runnable {
	...
	private static final int MESSAGE_SIMPLEJOY = 5;
	...
	public void run() {
		...
		while (ret >= 0) {
			...
			while (i < ret) {
				...
				case 0x9:
					if (len >= 3) {
						Message m = Message.obtain(mHandler, MESSAGE_SIMPLEJOY);
						m.obj = new SwitchMsg(buffer[i + 1], buffer[i + 2]);
						mHandler.sendMessage(m);
					}
					i += 3;
	...
	Handler mHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case MESSAGE_SIMPLEJOY:
				SwitchMsg k = (SwitchMsg) msg.obj;
				handleSimpleJoyMessage(k);
				break;
